...
def main():
    # single-source: список кодов берём из config/codes.json (порядок сохраняется).
    # Новичку НЕ нужно править этот файл — достаточно добавить ключ в codes.json (шаг 2).
    # Коды без data/final/{код}_structured.html молча пропускаются (analyze_code -> None).
