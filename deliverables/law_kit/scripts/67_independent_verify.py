...
# single-source: код-реестр и self-doc_id берём из config/codes.json (как audit).
# Новичку НЕ нужно править этот файл — достаточно добавить ключ в codes.json (шаг 2).
_REG = json.loads((ROOT / "config" / "codes.json").read_text(encoding="utf-8"))
_REG = {k: v for k, v in _REG.items() if not k.startswith("_")}
CODES = list(_REG.keys())
SELF = {k: v["doc_id"] for k, v in _REG.items()}
...
def main():
    ...
    for code in CODES:
        ...
        if not fp.exists():
            continue  # код объявлен в codes.json, но ещё не обработан — пропускаем
