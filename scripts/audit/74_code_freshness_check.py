...
# НАМЕРЕННЫЕ ДАТИРОВАННЫЕ исторические/переходные отсылки на отменённую редакцию.
# Видимая фраза НАЗЫВАЕТ конкретную старую редакцию по ДАТЕ («Кодекс РК от 25 декабря
# 2017 года "О налогах…"»), поэтому ремап на новую редакцию ЗАПРЕЩЁН (§4: видимая
# фраза != целевой акт). Цель архивно существует на adilet, подтверждено флагом Анары.
# (slug, docid) -> причина. НЕ путать с hist_links (там преемник == сам документ).
HISTORICAL_DATED = {
    ("tamozhenniy", "K1700000120"): "переходные положения ст.89/417: «статьёй 250/427 Кодекса РК от 25 декабря 2017 года \"О налогах…\"» — датированная отсылка на ред. 2017 (флаг Анары, §4)",
}
