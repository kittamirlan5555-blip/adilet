...
"""
    ...
    # ФИКС СКЛЕЕК (замечание Анары; диагноз night_notes §2): при обёртке
    # «пункт N статьи M» два соседних спана остаются как «N</a><a…>статьи» без
    # пробела → get_text склеивает «Nстатьи» (видно в цитате). В исходнике adilet
    # пробел ЕСТЬ (проверено на Z000000126_/Z030000461_: 0 склеек, N с пробелом),
    # т.е. это восстановление инварианта. Точечно вставляем пробел ТОЛЬКО на
    # границе «цифра</a><a…>стать» — прочий HTML не трогаем (гейт выше whitespace-
    # нечувствителен, поэтому и пропускал склейку, и пропустит возврат пробела).
    html_str = re.sub(r'(\d)(</a>)(<a\b[^>]*>стать[яиею])', r'\1\2 \3', html_str)
    if "<head>" in html_str:
