...
def main():
    ...
    L = ["# QUALITY — структуризация корпуса по схеме шефа", "",
         "Формат: чанковый jsonl, принятый в мае (эталон send_chef/"
         "chunks_ugolovniy.jsonl): id, text c заголовочным префиксом, meta "
         "(code, doc_id, article, unit_type/number, status, hier_id вида "
         "UKCH1R1ST1P1, hier_path). Источник — текущие _structured "
         "(read-only).", "",
    ...
    for r in results:
        for n in r["notes"]:
            L.append(f"- {r['slug']}: {n}")
    L += ["", "## Сложные случаи (как в ПРИМЕРАХ май-пакета)", "",
